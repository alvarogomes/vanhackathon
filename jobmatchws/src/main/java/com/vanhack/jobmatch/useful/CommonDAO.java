...
public class CommonDAO<T>{
	...
	public void set(Class<T> persistentClass) {
	  ...
	  public Object findById(final Object id, Class myClass) {
			try {
				Object instance = getEntityManager().find(myClass, id);
				return instance;
			} catch (RuntimeException re) {
				throw re;
			}
	  ...
	  /**
		...
		public List query(String sentenceJpa, Class myClass) {
			try {
				Query query = getEntityManager().createQuery(sentenceJpa,myClass);
				return query.getResultList();
			} catch (RuntimeException re) {
				throw re;
			}	
